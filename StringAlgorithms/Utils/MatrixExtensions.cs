...
namespace StringAlgorithms
{
    public static class MatrixExtensions
    {
        ...
        public static int[,] FillRowWithIntValue(this int[,] matrix, int rowIdx, ValueFun fun)
        {
            for (int i = 1; i < matrix.GetLength(1); ++i)
            {
                matrix[rowIdx, i] = fun(i);
            }
            return matrix;
        }

        public static int[,] FillColumnWithIntValue(this int[,] matrix, int columnIdx, ValueFun fun)
        {
            for (int i = 1; i < matrix.GetLength(0); ++i)
            {
                matrix[i, columnIdx] = fun(i);
            }
            return matrix;
        }
